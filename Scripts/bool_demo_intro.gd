...
var messages = ["This demo shows how ints are used.",
				"\"Bool\" is short for \"Boolean Expression\", which is just a True/False value.",
				"These are typically used to control if/when certain sections of the code runs.",
				"In the following example, you'll toggle a boolean value to control if the background color pulses.",
				"Press the SPACE BAR to toggle this pulsing on or off.",
				"In the background, you're switching the value of this bool.",
				"The background only pulses when the value is true",
...
func fade_in_text() -> void:
	...
	print("Advance timer started")
	$AdvanceTimer.start()
	await $AdvanceTimer.timeout
	print("Advance timer finished")
	fade_out($Text, 1)


func fade_out(component: Control, fade_time: float) -> void:
	print("Fade out started: {0}".format([component.name]))
	...
	await alpha_tween.finished
	print("Fade out completed: {0}".format([component.name]))
...
func move_to_demo() -> void:
	get_tree().change_scene_to_file("res://Scenes/bool_demo.tscn")
